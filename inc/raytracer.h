...
class RayTracer{
    ...
    public:
        ...
        RayTracer(int imageWidth, int imageHeight, int superSampling): imageWidth(imageWidth), imageHeight(imageWidth), superSampling(superSampling), hostScene(imageWidth, imageHeight, superSampling), depth(1)
        {
                image = new RGB[imageWidth*imageHeight];
                hostLight.position = Vec3d(1.0, 1.0, 1.0);
                hostLight.orientation = Vec3d(10.0, 10.0, -10.0);
