...
struct Light_h{
    ...
    __host__ __device__
    Light_h(): color(1.0, 1.0, 1.0), position(1.0,1.0,1.0), orientation(.5, -1, -1), width(1), height(1), radius(0.5), constantTerm(0), linearTerm(0.01), quadraticTerm(0.001) {} 
};
    ...
    public:
        ...
        __device__
            Light(Scene_d* scene, const Light_h& light): scene(scene) {
                ...
                height      = light.height      ;
                radius      = light.radius      ;
                constantTerm =light.constantTerm;
                linearTerm  = light.linearTerm  ;
                quadraticTerm =light.quadraticTerm;
